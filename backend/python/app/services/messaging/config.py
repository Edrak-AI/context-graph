...
class MessagingEnvConfig:
    ...
    def fair_scheduling_enabled(self) -> bool:
        ...
        Default off: enabling it buffers messages in-process and dispatches
        them out of broker order, which switches the Kafka consumer from
        "commit offset+1 per message" to a contiguous commit watermark. That
        is a protocol change, not a tuning knob, so an upgrade never turns it
        on by itself. Off is the exact pre-existing FIFO behavior with zero
        buffering overhead.
        """
    ...
    def fair_scheduling_lane_count(self) -> int:
        ...
        ``1`` (the default) means no laning: the producer is left unwrapped
        and the publish path is exactly what it is today. Above 1, records are
        placed by fairness key so one key's backlog never sits in front of
        another's -- which is what lets a full buffer pause a single lane
        instead of stalling every key behind it.
