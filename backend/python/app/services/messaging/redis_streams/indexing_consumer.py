...
class IndexingRedisStreamsConsumer(IMessagingConsumer):
    ...
    def __init__(
        ...
        self._deferred_entries: deque[
            tuple[str, str, dict[str, str], StreamMessage, FairnessKey]
        ] = deque()
    ...
    async def __has_unheld_pending(self, topic: str) -> bool:
        ...
        try:
            ...
            self.logger.debug("Could not inspect pending list on %s: %s", topic, e)
            return True
    ...
    async def __drain_deferred(self) -> None:
        ...
        for entry in parked:
            stream_name, message_id, fields, parsed, key = entry
    ...
    def __sweep_stale_buffered(self) -> None:
        ...
        if dropped:
            ...
            metrics.record_dwell_exceeded("redis", len(dropped))
            self.logger.warning(
                "Released %d entry(ies) buffered longer than %.0fs back to the "
    ...
    async def __read_phase(self) -> None:
        ...
        results = await self.redis.xreadgroup(  # type: ignore
            ...
            consumername=self.consumer_name,
            streams=streams,
            count=min(max(1, self.config.batch_size), buffer_room),
